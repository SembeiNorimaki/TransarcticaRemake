...
class TradeScene {
  constructor(locationName, board) {
    ...
    this.tileBoard = new TileBoard(board, this.tileHalfSize);
    this.infoPanel = new InfoPanel();
    this.conversationPanel = new ConversationPanel();

    this.enterSequence = true;
    this.exitSequence = false;
    this.camera = new Camera(Geometry.boardToCamera(createVector(15,15), this.tileHalfSize));
  ...
  initialize() {
    ...
    this.horizontalTrain.setPosition(createVector(-5, 16));
  ...
  update() {
    // Enter sequence
    if (this.enterSequence && this.horizontalTrain.position.x > 0) {
