...
class HorizontalTrain {
  ...
  update() {
    ...
    this.wagons[0].setPos(this.currentPosition);  
    this.currentPosition.sub(this.wagons[0].halfSize.x+2, 0);
    ...
    // this.wagons[0].update();
    // this.wagons[0].setPos(this.position);

    // for (let i=1; i<this.wagons.length; i++) {
    //   this.wagons[i].update();
    //   auxPos.x -= this.wagons[i].halfSize.x*2;
  ...
  show(cameraPosition) {
    for (let [i, wagon] of this.wagons.entries()) {
      ...
      //if (["Cannon", "Machinegun", "Cannon_vu", "Machinegun_vu"].includes(wagon.name)) {
        ...
        //this.wagons[i].showReloadBar(cameraPosition);
      //}
